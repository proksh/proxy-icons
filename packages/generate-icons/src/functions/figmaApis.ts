...
export interface FigmaFileImageResponse
...
export async function getFigmaDocument(): Promise<Document> {
  const resp = await fetch(
    `${baseUrl}/v1/files/${process.env.FIGMA_FILE_KEY}`,
    {
      headers: new Headers({
...
export function getIcons(iconsCanvas: Canvas): Icons {
  return iconsCanvas.children.reduce(
    (icons: Icons, categroryGroupFrame: any) => {
      // Find the icons Wrapper
      const iconsFrame: any = categroryGroupFrame.children.find(
        (frame: any) => frame.name === "Icons"
      ) as Node;
          ...
          icons[iconNode.id] = {
            ...
            type: (() => {
              ...
              handleError(
                new CodedError(
                  ERRORS.FIX_FIGMA,
                  `Some incorrect icon - ${iconName}, ${iconType}`,
                  true
                )
              );
...
export async function renderIdsToSvgs(ids: string[]): Promise<IconsSvgUrls> {
  const resp = await fetch(
    `${baseUrl}/v1/images/${process.env.FIGMA_FILE_KEY}?ids=${ids}&format=svg`,
    {
      headers: new Headers({
        "X-Figma-Token": process.env.FIGMA_ACCESS_TOKEN || "",
      }),
